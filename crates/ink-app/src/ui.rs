...
fn icon_dropper(p: &egui::Painter, c: Pos2, s: f32, col: Color32) {
...
fn draw_arc(p: &egui::Painter, c: Pos2, radius: f32, t: f32, value: f32, label: &str, grad: &dyn Fn(f32) -> [u8; 3]) {
    ...
    for i in 0..n {
        ...
        let stroke = Stroke::new(9.0, Color32::from_rgba_unmultiplied(col[0], col[1], col[2], alpha));
        p.line_segment(
    ...
    p.circle_filled(hp, 8.0, Color32::from_rgba_unmultiplied(255, 255, 255, alpha));
    p.circle_stroke(hp, 8.0, Stroke::new(2.0, Color32::from_rgba_unmultiplied(70, 70, 70, alpha)));
    // Cajita de valor por fuera del tirador.
    let bp = c + dir(ha) * (radius + 24.0);
...
/// Celda del panel: vista previa arriba + nombre abajo. Devuelve true si se clicó.
fn item_cell(ui: &mut egui::Ui, name: &str) -> bool {
    let (rect, resp) = ui.allocate_exact_size(egui::vec2(114.0, 82.0), egui::Sense::click());
    let p = ui.painter_at(rect);
    if resp.hovered() {
        p.rect_filled(rect, egui::CornerRadius::same(8), Color32::from_gray(230));
    }
    draw_preview(&p, rect.center() - egui::vec2(0.0, 12.0), name);
    p.text(
...
const SP_R0: f32 = R_OUT + 74.0; // radio interior del primer anillo
...
/// Encabezado de seccion grande con chevron (solo visual).
fn section_head(ui: &mut egui::Ui, title: &str) {
    ui.add_space(6.0);
    ui.horizontal(|ui| {
        ui.label(egui::RichText::new(title).size(24.0).strong().color(Color32::from_gray(20)));
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ui.label(egui::RichText::new("⌄").size(20.0).color(Color32::from_gray(120)));
        });
    });
    ui.add_space(8.0);
}
...
fn draw_collapse_ripple(ctx: &egui::Context, center: Pos2, t: f32, color: Color32) {
    ...
    let p = ctx.layer_painter(egui::LayerId::new(egui::Order::Middle, egui::Id::new("wheel_ripple")));
...
fn settings_panel(
    ...
    egui::Window::new("ajustes")
        ...
        .show(ctx, |ui| {
            // --- Encabezado: X | pestanas | info ---
            ui.horizontal(|ui| {
                if ui.add(egui::Button::new(egui::RichText::new("✕").size(18.0)).frame(false)).clicked() {
                    state.show_settings = false;
                }
                ui.add_space(10.0);
                let tab = state.settings_tab;
                if ui
                    .selectable_label(tab == SettingsTab::Workspace, egui::RichText::new("Área de trabajo").size(17.0).strong())
                    .clicked()
                {
                ...
                ui.add_space(8.0);
                if ui
                    .selectable_label(tab == SettingsTab::Interaction, egui::RichText::new("Interacción").size(17.0).strong())
                    .clicked()
                {
                    state.settings_tab = SettingsTab::Interaction;
                }
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ui.label(egui::RichText::new("ⓘ").size(16.0).color(Color32::from_gray(140)));
                });
            });
            ui.add_space(6.0);
            ui.separator();
            ...
            egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
                match state.settings_tab {
                    SettingsTab::Workspace => workspace_tab(ui, cfg, state, brush),
...
fn workspace_tab(ui: &mut egui::Ui, cfg: &mut crate::settings::Settings, state: &mut UiState, brush: &mut Brush) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            let _ = ui.link(egui::RichText::new("Editar cuadrícula").color(Color32::from_rgb(40, 110, 210)));
        });
    });
    ...
    let grids = [
        ...
        (GridKind::P2, "2 puntos", true),
        (GridKind::P3, "3 puntos", true),
    ];
    ui.horizontal_wrapped(|ui| {
...
pub fn build_panel(
        ...
        .show(ctx, |ui| {
            egui::Frame::popup(ui.style()).show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .selectable_label(grid_on, egui::RichText::new("▦").size(16.0))
                        .on_hover_text("Mostrar/ocultar cuadricula")
                        .clicked()
                    {
                        // Recordamos el ultimo tipo para alternar sin perderlo.
                        cfg.grid = if grid_on {
                            state.last_grid = state.last_grid; // no-op, claridad
                    ...
                    if ui
                        ...
                        .on_hover_text("Ajustes del area de trabajo")
                        .clicked()
                    {
    ...
    // El panel de ajustes (modal) se dibuja por encima de todo al final.
    ...
    if cfg.tool_ui == crate::settings::ToolUi::Wheel {
        ...
        draw_collapse_ripple(ctx, wheel_center, collapse_t, c32(brush.color));
    ...
    egui::Area::new(egui::Id::new("wheel"))
        ...
        .show(ctx, |ui| {
            ...
            wheel_center = c;
            if state.collapsed {
                // Solo el punto de color; al tocarlo se reabre la rueda.
                let p = ui.painter();
            ...
            let line = Stroke::new(1.0, Color32::from_gray(226));
            let ink = Color32::from_gray(70);

            // Anillo exterior + segmento seleccionado.
            p_circle(ui, c, R_OUT, ring_bg);
            let sa = state.selected_seg as f32 * SEG_DEG + rot;
            fill_sector(ui.painter(), c, R_MID, R_OUT, sa - SEG_DEG / 2.0, sa + SEG_DEG / 2.0, Color32::from_gray(28));
            for i in 0..N_SEG {
                let a = i as f32 * SEG_DEG + SEG_DEG / 2.0 + rot;
                ui.painter().line_segment([c + dir(a) * R_MID, c + dir(a) * R_OUT], line);
            }
            ui.painter().circle_stroke(c, R_OUT, Stroke::new(1.5, Color32::from_gray(215)));
            // Donut gris.
            p_circle(ui, c, R_MID, gray);

            // Resaltar la zona del control con panel abierto.
            let hl = Color32::from_rgb(206, 224, 248);
            match state.popup {
                Popup::Size => fill_sector(ui.painter(), c, R_HOLE, R_MID, -60.0, 60.0, hl),
                Popup::Opacity => fill_sector(ui.painter(), c, R_HOLE, R_MID, 60.0, 150.0, hl),
                Popup::Smoothing => fill_sector(ui.painter(), c, R_HOLE, R_MID, 210.0, 300.0, hl),
                Popup::None => {}
            }

            let p = ui.painter().clone();

            // Iconos / muestras / numeros por segmento.
            let ric = (R_MID + R_OUT) / 2.0 + 1.0;
            for i in 0..N_SEG {
                let sel = i == state.selected_seg;
                let col = if sel { Color32::WHITE } else { ink };
                let ipos = c + dir(i as f32 * SEG_DEG + rot) * ric;
                match state.slots[i] {
                    SlotItem::Empty => {
                        icon_x(&p, ipos, 10.0, if sel { Color32::WHITE } else { Color32::from_gray(125) });
                    }
            ...
            icon_grip(&p, c + egui::vec2(-26.0, -(R_HOLE + 14.0)), 7.0, Color32::from_gray(120));
            icon_brush_sample(&p, c + egui::vec2(-rside, -1.0), 8.0, Color32::from_gray(110));
            icon_opacity(&p, c + egui::vec2(rside, -1.0), 7.0, Color32::from_gray(110));
            // Boton de colapsar (abajo): oculta la rueda dejando solo el punto de color.
            icon_collapse(&p, c + egui::vec2(0.0, rside), 6.5, Color32::from_gray(120));

            // Circulo de color central.
            p.circle_filled(c, R_HOLE, c32(brush.color));
            p.circle_stroke(c, R_HOLE, Stroke::new(2.0, Color32::from_gray(245)));
            ...
            p.text(c + egui::vec2(12.0, -(R_HOLE + 14.0)), Align2::CENTER_CENTER,
                cfg.format_measure(brush.width), FontId::proportional(11.0), Color32::from_gray(55));
            p.text(c + egui::vec2(-(R_HOLE + 18.0), R_HOLE + 8.0), Align2::CENTER_CENTER,
                format!("{:.0}%", brush.smoothing * 100.0), FontId::proportional(11.0), Color32::from_gray(85));
            p.text(c + egui::vec2(R_HOLE + 18.0, R_HOLE + 8.0), Align2::CENTER_CENTER,
                format!("{:.0}%", brush.opacity * 100.0), FontId::proportional(11.0), Color32::from_gray(85));

            // Deshacer / Rehacer.
            let undo_p = c + egui::vec2(-(R_OUT + 26.0), -28.0);
            let redo_p = c + egui::vec2(-(R_OUT + 26.0), 30.0);
            icon_curved_arrow(&p, undo_p, 12.0, Color32::from_gray(if stats.can_undo { 40 } else { 185 }), false);
            icon_curved_arrow(&p, redo_p, 12.0, Color32::from_gray(if stats.can_redo { 40 } else { 185 }), true);
        ...
        egui::Area::new(egui::Id::new("brush_panel"))
            ...
            .show(ctx, |ui| {
                egui::Frame::popup(ui.style()).show(ui, |ui| {
                    ...
                    ui.add_space(2.0);
                    ui.label(egui::RichText::new("Básico").strong());
                    ui.horizontal_wrapped(|ui| {
                    ...
                    ui.add_space(8.0);
                    ui.label(egui::RichText::new("Herramientas").strong());
                    ui.horizontal_wrapped(|ui| {
    ...
    let copic_t = ctx.animate_bool_with_time(egui::Id::new("m_copic"), state.show_colors && state.color_mode == ColorMode::Copic, 0.2);
    let hsl_t = ctx.animate_bool_with_time(egui::Id::new("m_hsl"), state.show_colors && state.color_mode == ColorMode::Hsl, 0.2);
    let rgb_t = ctx.animate_bool_with_time(egui::Id::new("m_rgb"), state.show_colors && state.color_mode == ColorMode::Rgb, 0.2);
    if state.show_colors || copic_t.max(hsl_t).max(rgb_t) > 0.01 {
        egui::Area::new(egui::Id::new("color_picker"))
            ...
            .show(ctx, |ui| {
                let screen = ctx.content_rect();
                let (_r, resp) = ui.allocate_exact_size(screen.size(), egui::Sense::click_and_drag());
                ...
                if copic_t > 0.01 {
                    ...
                    for (ci, col) in cols.iter().enumerate() {
                        let amid = SP_START + (ci as f32 + 0.5) * col_w;
                        let a0 = SP_START + ci as f32 * col_w + 0.4;
                        let a1 = SP_START + (ci as f32 + 1.0) * col_w - 0.4;
                        for (ri, &(code, rgb)) in col.iter().enumerate() {
                            let r_in = SP_R0 + ri as f32 * grow + 1.0;
                            let r_out = SP_R0 + (ri as f32 + 1.0) * grow - 1.0;
                            let quad = vec![c + dir(a0) * r_out, c + dir(a1) * r_out, c + dir(a1) * r_in, c + dir(a0) * r_in];
                            let cc = rgb_to_color(rgb);
                            p.add(Shape::convex_polygon(quad, Color32::from_rgba_unmultiplied(cc.r(), cc.g(), cc.b(), alpha), Stroke::NONE));
                            if copic_t > 0.65 {
                ...
                if hsl_t > 0.01 {
                    ...
                    draw_arc(&p, c, radii[0], hsl_t, h / 360.0, &format!("{:.0}°", h), &|x| to3(hsl_to_rgb(x * 360.0, 1.0, 0.5)));
                    draw_arc(&p, c, radii[1], hsl_t, s, &format!("{:.0}%", s * 100.0), &|x| to3(hsl_to_rgb(h, x, l.clamp(0.25, 0.75))));
                    draw_arc(&p, c, radii[2], hsl_t, l, &format!("{:.0}%", l * 100.0), &|x| to3(hsl_to_rgb(h, s, x)));
                }
                // --- RGB ---
                if rgb_t > 0.01 {
                    let (r, g, b) = (brush.color[0], brush.color[1], brush.color[2]);
                    draw_arc(&p, c, radii[0], rgb_t, r, &format!("{}", (r * 255.0) as i32), &|x| [(x * 255.0) as u8, 0, 0]);
                    draw_arc(&p, c, radii[1], rgb_t, g, &format!("{}", (g * 255.0) as i32), &|x| [0, (x * 255.0) as u8, 0]);
                    draw_arc(&p, c, radii[2], rgb_t, b, &format!("{}", (b * 255.0) as i32), &|x| [0, 0, (x * 255.0) as u8]);
                }

                // --- Etiquetas COPIC/HSL/RGB + cuentagotas ---
                let lx = c.x + R_OUT + 24.0;
                let lbl = |y: f32, txt: &str, active: bool| {
                    p.text(
                        egui::pos2(lx, c.y + y),
                        Align2::LEFT_CENTER,
                        txt,
                        FontId::proportional(if active { 18.0 } else { 15.0 }),
                        if active { Color32::from_gray(40) } else { Color32::from_gray(125) },
                    );
                };
                lbl(-28.0, "COPIC", state.color_mode == ColorMode::Copic);
                lbl(0.0, "HSL", state.color_mode == ColorMode::Hsl);
                lbl(24.0, "RGB", state.color_mode == ColorMode::Rgb);
                let drop_pos = egui::pos2(lx + 14.0, c.y + 54.0);
                icon_dropper(&p, drop_pos, 13.0, if state.eyedropper { Color32::from_rgb(40, 120, 220) } else { Color32::from_gray(95) });
                ...
                if state.show_colors {
                    ...
                    if resp.clicked() {
                        if let Some(pp) = resp.interact_pointer_pos() {
                            let near = |y: f32| (pp - egui::pos2(lx + 24.0, c.y + y)).length() < 42.0;
                            if near(-28.0) {
                            ...
                            } else if (pp - drop_pos).length() < 20.0 {
                        ...
                        if let Some(pp) = resp.interact_pointer_pos() {
                            if let Some((arc, t)) = arc_hit(c, pp, &radii) {
                                match state.color_mode {
                                    ColorMode::Hsl => set_hsl(brush, arc, t),
                                    ColorMode::Rgb => brush.color[arc] = t,
                                    ColorMode::Copic => {}
                                }
                            }
                        }
    ...
    egui::Area::new(egui::Id::new("options"))
        ...
        .show(ctx, |ui| {
            egui::Frame::popup(ui.style()).show(ui, |ui| {
                ...
                let _ = ui.button("≡   Capas");
                if ui.button("⠿   Precisión").clicked() {
                    state.show_settings = true;
                    state.settings_tab = SettingsTab::Workspace;
                }
                ui.separator();
