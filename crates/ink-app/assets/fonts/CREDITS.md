# Fuentes

Estilo editorial de la interfaz. Ambas con licencia **SIL Open Font License 1.1 (OFL)**:

- **Hanken Grotesk** (HankenGrotesk-Regular.ttf, HankenGrotesk-Bold.ttf) — texto/titulos de la UI.
  © Alfredo Marco Pradil. https://github.com/Hanken-Design/Hanken-Grotesk
- **JetBrains Mono** (JetBrainsMono-Regular.ttf) — etiquetas monoespaciadas (estilo devtool).
  © JetBrains. https://github.com/JetBrains/JetBrainsMono
- **Spectral** (Spectral-Regular/SemiBold/Italic.ttf) — titulo/subtitulo del tema "Cuaderno"
  (serif editorial). © Production Type. https://github.com/productiontype/Spectral

## Fuentes de escritura (modo documento)

Para escribir textos con teclado en la hoja. Todas **OFL 1.1**:

- **Lora** (Lora-Regular.ttf) — serif moderna para cuerpo de texto. © Cyreal.
- **Merriweather** (Merriweather-Regular.ttf) — serif robusta y muy legible. © Sorkin Type.
- **EB Garamond** (EBGaramond-Regular.ttf) — serif clasica/literaria. © Georg Duffner / Octavio Pardo.
- **Atkinson Hyperlegible** (AtkinsonHyperlegible-Regular.ttf) — sans de alta legibilidad. © Braille Institute.
- **Source Sans 3** (SourceSans3-Regular.ttf) — sans humanista limpia. © Adobe.

Subconjunto latino (TTF estatico) via Fontsource. Se incrustan con `include_bytes!` (ver
`setup_fonts` en `main.rs`); el sistema (Segoe UI / DejaVu) queda como respaldo.
